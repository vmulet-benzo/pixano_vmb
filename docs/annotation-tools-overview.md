...
flowchart TB
    ...
    BUILDERS["<b>PAYLOAD_BUILDERS</b> — per kind<br/>how an annotation becomes REST bodies<br/>(bbox · bbox3d · keypoints)"]

    QUEUE["<b>MutationQueue</b><br/>pending 'TODO for the server' notes<br/>flush order: entities → annotations → deletes"]

    API["REST API<br/>/bboxes /bbox3ds /keypoints /entities"]
...
4. **Tests** — renderer `sync()` against a fake `Scene2DContext` (pattern:
   `tools/__tests__/selectTool2D.test.ts`, Konva mocked as in
   `kinds/2d/keypoints/__tests__/drawKeypointsTool.test.ts`), plus pure
   projection-math tests (pattern: `__tests__/coordinateTransforms.test.ts`).
