...
1. Create `lib/annotations/kinds/<kind>/` with: geometry type, payload builder,
   renderer(s), tool(s).
2. Register the payload builder, renderer(s) and tool(s) in the registries.
3. Nothing else changes: no widget edits, no mode-union edits, no queue edits.
