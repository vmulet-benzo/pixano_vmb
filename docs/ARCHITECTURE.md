...
> annotations still lives in the view extension's `addRecordSeed`
> (`ImageExtension` lists bboxes and keypoints explicitly and returns them via
> `seed.annotations`; the loader merges every seed's contributions into the
> shared record collection, deduplicating by id). Listing goes through the
> generic `gateway.listAnnotations(resource)`, so no new gateway methods are
> needed per kind, but a per-kind seed-loader registry is the next seam to
> build when a third 2D kind lands.
