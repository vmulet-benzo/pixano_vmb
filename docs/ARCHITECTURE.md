...
many more annotation kinds (mask RLE, keypoints, polylines, …), so the cost of
...
interface LocalAnnotation<G = unknown> {
  id: string;
  entityId: string;
  kind: AnnotationKind;     // "bbox" | "bbox3d" | "keypoints" | "mask" | …
...
and delete-with-pending-mutation-drop. Both 2D and 3D widget storages hold an
`AnnotationCollection`; widgets stop owning lifecycle logic.
...
> Known gap (found while validating with keypoints): *loading* persisted
> annotations into a record seed still lives in the view extension's
> `addRecordSeed` (`ImageExtension` lists bboxes and keypoints explicitly).
> Listing goes through the generic `gateway.listAnnotations(resource)`, so no
> new gateway methods are needed per kind, but a per-kind seed-loader registry
> is the next seam to build when a third 2D kind lands.
...
| 5 | Implement 2D keypoints end-to-end as validation; then mask RLE |
